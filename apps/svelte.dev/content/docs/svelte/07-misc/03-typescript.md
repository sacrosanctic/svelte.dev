...
In case you're writing a component that wraps a native element, you may want to expose all of the attributes of the underlying element to the user. In that case, use (or extend from) one of the interfaces provided by `svelte/elements`. Here's an example for a `Button` component:
